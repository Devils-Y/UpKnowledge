...
public class SelectedVideo4HomeAdapter extends BaseAdapter<SelectedVideoBean> {

     public SelectedVideo4HomeAdapter(Context context, List<SelectedVideoBean> data) {
          super(context, data, R.layout.item_video);
     }
     ...
     @Override
     protected void convert(BaseViewHolder holder, SelectedVideoBean item, int position) {
          holder.setText(R.id.title, item.getTitle())
                    .setText(R.id.collection, String.valueOf(item.getConsumption().getCollectionCount()))
                    .setText(R.id.share, String.valueOf(item.getConsumption().getShareCount()))
                    .setText(R.id.profile, new StringBuilder().append(new String() == "" ? null : item.getProvider().getName())
